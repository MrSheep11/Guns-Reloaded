...
$execute unless data storage mcfentitymap:maps $(path)[{UUID: "$(UUID)"}].values.$(key) unless score #MCFEM_GLOBAL mcfem.suppress_errors matches 1 run return run function mcfentitymap:message/error {message: "Entity does not have attribute $(key)!"}
